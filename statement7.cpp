...
typedef pair<BigInt,BigInt> pBI;
typedef map<pBI, int> mpBI;

int MAXN = 5000;

bool compare(pBI p1, pBI p2){
...
int main(){
    ...
    sort(table2,table2 + n,compare);
    FOR(i, 0, n){
        // cout << table2[i] << endl;
        mapBI[table2[i]] = i + 1;
    }
    FOR(i, 0, n){
        cout << mapBI[table[i]] << endl;
    }
    return 0;
}